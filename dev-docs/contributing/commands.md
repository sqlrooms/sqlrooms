...
AI and skill runtimes should discover commands with compact search before
loading schemas:

1. `search_commands` for intent-based discovery.
2. `get_command` for the selected command's full metadata and input schema.
3. `execute_command` for the confirmed action.

Keep `list_commands` available for broad exploration and debugging, but avoid
repeated full command listings in routine model flows. High-risk or
confirmation-required commands should remain discoverable and carry clear
metadata; `execute_command` requires an explicit `confirmed: true` before
running them. Skill runtimes should pass trace metadata such as `aiSessionId`,
`skillId`, `toolCallId`, and `traceId` so command callbacks can surface the
validated input and result code/message in future devtools traces.
...
| ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Generic block-document text/chart/block creation              | `BlockDocumentAiAdapter.addBlock` in `packages/documents` tools, with the CLI adapter appending through `state.blockDocuments.appendBlocks`                   | Should become command-backed                                                 | Stage 2: route the adapter through `document.append-blocks` / `document.create-chart-block` while preserving the generic `BlockDocument` API boundary. |
...
| Workspace artifact create/select                              | CLI `createDashboardCommands` exposes `*.create-artifact` commands and `artifact.select`                                                                      | Already a good command baseline                                              | Keep as the model for result payloads with stable artifact IDs.                                                                                        |
| Workspace artifact rename                                     | Topbar, document title editor, sidebar tabs, and app-builder flows call `artifacts.renameArtifact` directly                                                   | Should become a command                                                      | Stage 4: add `artifact.rename` with validation, no-op result code, previous/new title data, and existing artifact-type `onRename` hooks preserved.     |
...
| CLI document dashboard/data-table/html-app/map block creation | Document AI composition and direct document map/local import flows create stateful blocks through block-document slice methods and feature-specific callbacks | Should become CLI commands                                                   | Stage 5: add CLI-owned document commands that wrap lower-level document commands without moving document product policy into shared packages.          |
| Local file loading into documents                             | `useLocalFileLoader` loads tables, ensures a document, and appends a data-table stateful block directly                                                       | Should become command-backed after document block commands exist             | Stage 5: keep file import/loading concerns separate, then route the document block addition through the document command.                              |
