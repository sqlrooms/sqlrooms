...
def get_embedding_metadata(
    ...
    try:
        ...
        results = conn.execute("SELECT key, value FROM embedding_metadata").fetchall()
        ...
        metadata = {
            ...
            "chunking": {
                ...
                "include_headers": flat_metadata.get("include_headers", "false").lower()
                == "true",
                "header_weight": int(flat_metadata.get("header_weight", "0")),
            },
            "source_documents": {
                "total_documents": int(
                    flat_metadata.get("total_source_documents", "0")
                ),
                "unique_files": int(flat_metadata.get("unique_files", "0")),
                "total_characters": int(
                    flat_metadata.get("source_total_characters", "0")
                ),
            },
            "chunks": {
                ...
                "total_characters": int(
                    flat_metadata.get("chunks_total_characters", "0")
                ),
            },
            "capabilities": {
                "hybrid_search": flat_metadata.get(
                    "hybrid_search_enabled", "false"
                ).lower()
                == "true",
                "fts_enabled": flat_metadata.get("fts_enabled", "false").lower()
                == "true",
                "source_documents_stored": flat_metadata.get(
                    "source_documents_stored", "false"
                ).lower()
                == "true",
            },
        }
...
def validate_embedding_model(
    db_path: str, model_name: str, expected_dimensions: Optional[int] = None
) -> bool:
    """
    ...
    if stored_model != model_name:
        ...
        print("  Results may be poor or meaningless.")
        return False
    ...
    if expected_dimensions and stored_dims != expected_dimensions:
        ...
        print("  Results will be meaningless.")
        return False
...
def get_source_documents(
    chunk_ids: List[str], db_path: str = "generated-embeddings/sqlrooms_docs.duckdb"
) -> List[Dict[str, Any]]:
    """
...
def hybrid_query(
    ...
    try:
        ...
        vector_results = conn.execute(
            vector_query, [query_embedding, vector_top_k]
        ).fetchall()
        ...
        try:
            ...
            fts_results = conn.execute(
                fts_query, [query_text, query_text, fts_top_k]
            ).fetchall()
            ...
            if (
                "fts_main_documents" in error_msg
                or "match_bm25" in error_msg
        ...
        if use_rrf:
            ...
            if verbose:
                print(
                    f"Combining results with weights (vector: {vector_weight}, fts: {1 - vector_weight})..."
                )
        ...
        if include_source_docs and final_results:
            ...
            for result in final_results:
                ...
                """
                doc_id_result = conn.execute(
                    doc_id_query, [result["node_id"]]
                ).fetchone()
            ...
            if verbose:
                ...
                print(
                    f"  Linked {docs_found}/{len(final_results)} chunks to source documents"
                )
