...
def extract_title_from_markdown(
    text: str, metadata_json: Optional[str] = None, fallback: str = ""
) -> str:
    """
            ...
            if valid_paths:
                ...
                try:
                    ...
                    for parent, count in parent_counts.most_common():
                        if (
                            count >= len(valid_paths) * 0.8
                        ):  # 80% of paths share this parent
                            try:
...
def load_embeddings_from_duckdb(db_path: str) -> pd.DataFrame:
    ...
    if "documents" not in table_names:
        raise ValueError(
            f"Table 'documents' not found in {db_path}. Available tables: {table_names}"
        )
...
def generate_umap_embeddings(
    ...
    """
    print("🗺️  Generating UMAP embeddings...")
...
def build_link_graph(
    ...
    matched_pct = (
        (matched_links / total_markdown_links * 100) if total_markdown_links > 0 else 0
    )

    print(
        f"✓ Found {total_markdown_links} markdown links in {chunks_with_links} chunks"
    )
...
def process_embeddings(
    ...
    for idx, row in df.iterrows():
        ...
        title = extract_title_from_markdown(
            row["text"], metadata_json=row["metadata_"], fallback=filename or "Untitled"
        )
        titles.append(title)
    ...
    if extract_links:
        try:
            ...
            print(
                f"   Built lookup index with {len(filename_to_idx)} path variants for {len(filenames)} documents"
            )

            # Build link graph
            outgoing_links, incoming_links = build_link_graph(
                df, filename_to_idx, filepaths
            )

            # Calculate metrics
            outdegree, indegree = calculate_graph_metrics(
                len(df), outgoing_links, incoming_links
            )
  ...
  generate-umap-embeddings embeddings.duckdb --preview 100
    ...
    try:
        ...
        if "outdegree" in result_df.columns and result_df["outdegree"].sum() > 0:
            ...
            print(
                f"  Documents with outgoing links: {(result_df['outdegree'] > 0).sum()}"
            )
            print(
                f"  Documents with incoming links: {(result_df['indegree'] > 0).sum()}"
            )
            ...
            if result_df["indegree"].max() > 0:
                ...
                top_referenced = result_df.nlargest(5, "indegree")[
                    ["title", "indegree"]
                ]
