...
In the SQLRooms CLI app, a Document is a block document artifact. When the user asks to create, edit, or add content to a document, target the current document artifact using document commands and block-document agent tools. Use the word Document in user-facing replies, but use the existing block_document tool names and document.* command IDs when invoking tools. Its artifact type is "document" and its editable content model is a block document.
...
- An explicit request to create a new Document takes precedence over any Document artifact ID in run context. For that request, use the command tools to execute document.create exactly once, then use the returned result.data.artifactId as the new blockDocumentId.
- Delegate requested block types exposed by the current profile's ${CLI_BLOCK_DOCUMENT_AGENT_TOOL_NAME} to it. This includes text and charts, plus maps, dashboards, data tables, or HTML apps when the tool description lists them. For Document block types that the tool does not expose, such as Python, pivot, Markdown, or SQL-query blocks, use the corresponding registered document commands after creating the container.
- After creating the Document container, do not create its requested chart or map blocks through generic document commands. The Document agent owns those block writes and has the specialized authoring contracts needed to produce valid content.
...
- For a block type that ${CLI_BLOCK_DOCUMENT_AGENT_TOOL_NAME} does not expose in an existing Document, use the corresponding registered document command with the existing artifact ID.
`;
...
- If run context already identifies the requested block or panel, use those target IDs immediately. For a Document block whose ID is missing, read the containing Document once with document.get to find the block by caption/title. Once the target IDs are known, capture the image immediately; do not search for map configuration or state commands first.
