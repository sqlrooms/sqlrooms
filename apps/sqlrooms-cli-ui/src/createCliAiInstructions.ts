...
- For document block reordering, call ${CLI_BLOCK_DOCUMENT_AGENT_TOOL_NAME} directly with the document ID and the requested move. It receives a fresh document snapshot with block IDs and current order; no preliminary document read or command discovery is needed. For a document-wide reorder, leave targetBlock unset and describe the move in intent. Only pass targetBlock for an explicit block-scoped target from run context.
