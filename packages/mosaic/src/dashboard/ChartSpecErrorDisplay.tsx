...
export const ChartSpecErrorDisplay: FC<ChartSpecErrorDisplayProps> = ({
  ...
  // More serious errors requiring attention
  if (error instanceof MissingTableError) {
    return (
      <>
        <div className="font-medium">{error.message}</div>
        <div className="text-xs">Select a table from the toolbar</div>
      </>
    );
  }

  if (error instanceof UnknownChartTypeError) {
    return (
      <>
        <div className="font-medium">{error.message}</div>
        <div className="text-xs">Try selecting a different chart type</div>
      </>
    );
  }
