import { DuckQueryError, createTableFromQuery, createViewFromFile, createViewFromRegisteredFile, dropAllFiles, dropAllTables, dropFile, dropTable, getDuckConn, getDuckTableSchemas, getDuckTables, } from '@sqlrooms/duckdb';
import { makeMosaicStack, removeMosaicNodeByKey } from '@sqlrooms/layout';
import { DEFAULT_MOSAIC_LAYOUT, DEFAULT_PROJECT_TITLE, DEFAULT_SQL_EDITOR_CONFIG, DataSourceTypes, ProjectPanelTypes, isMosaicLayoutParent, } from '@sqlrooms/project-config';
import { convertToUniqueColumnOrTableName, convertToUniqueS3FolderPath, downloadFile, generateUniqueName, getSignedFileUrl, splitFilePath, } from '@sqlrooms/utils';
...
export const INITIAL_BASE_PROJECT_STATE = {
    schema: 'main',
    tasksProgress: {},
    projectId: undefined,
    password: undefined,
...
export const INITIAL_BASE_PROJECT_CONFIG = {
    title: '',
    description: '',
    dataSources: [],
...
export function createProjectSlice(props) {
    const initialState = {
    ...
    const slice = (set, get) => {
        const projectState = {
            ...
            reinitialize: async (opts) => {
                const { project, isPublic = false, isReadOnly = false, captureException, password, } = opts ?? {};
                await get().reset();
            ...
            setProjectFolder: (projectFolder) => set((state) => produce(state, (draft) => {
            ...
            getProjectFolder() {
                const { projectFolder, projectConfig: { title }, } = get();
            ...
            removeSqlQueryDataSource: async (tableName) => {
                ...
                set((state) => produce(state, (draft) => {
                    draft.projectConfig.dataSources =
                        draft.projectConfig.dataSources.filter((d) => d.tableName !== tableName);
            ...
            async replaceProjectFile(projectFile) {
                ...
                const dataSource = get().projectConfig.dataSources.find((d) => d.type === DataSourceTypes.enum.file &&
                ...
                if (dataSource) {
                    ...
                    if (projectFile.duckdbFileName) {
                        const { rowCount } = await createViewFromRegisteredFile(projectFile.duckdbFileName, 'main', dataSource.tableName);
            ...
            async addProjectFile(projectFile, desiredTableName) {
                ...
                const tableName = desiredTableName ??
                    convertToUniqueColumnOrTableName(name, await getDuckTables());
                if (duckdbFileName) {
                    const { rowCount } = await createViewFromRegisteredFile(duckdbFileName, 'main', tableName);
            ...
            removeProjectFile(pathname) {
                set((state) => produce(state, (draft) => {
                    draft.projectFiles = draft.projectFiles.filter((f) => f.pathname !== pathname);
                    draft.projectConfig.dataSources =
                        draft.projectConfig.dataSources.filter((d) => d.type !== DataSourceTypes.Enum.file ||
                            d.fileName !== pathname);
                }));
                dropFile(pathname);
            },
            setProjectFileProgress(pathname, fileState) {
                set((state) => produce(state, (draft) => {
                    ...
                    const fileInfo = draft.projectFiles.find((f) => f.pathname === pathname);
            ...
            setProjectTitle: (title) => set((state) => produce(state, (draft) => {
            ...
            togglePanel: (panel, show) => {
                ...
                const result = removeMosaicNodeByKey(projectConfig.layout?.nodes, panel);
            ...
            async updateReadyDataSources() {
                ...
                const dataSources = projectConfig.dataSources;
                set({
                    dataSourceStates: dataSources.reduce((acc, ds) => {
        ...
        async function downloadProjectFiles(filesToDownload) {
            if (!filesToDownload.length)
                return;
            const { isPublic: isPublicProject, projectId, password, setProjectFileProgress, setTableRowCount, setTables, captureException, setTaskProgress, } = get();
            filesToDownload.forEach((ds) => {
                ...
                set((state) => produce(state, (draft) => {
                    ...
                    const index = draft.projectFiles.findIndex((f) => f.pathname === fileName);
                    if (index >= 0) {
            ...
            const loadedFiles = await Promise.all(filesToDownload.map(async (ds) => {
                const fileName = ds.type === DataSourceTypes.Enum.file ? ds.fileName : ds.url;
                try {
                    const url = ds.type === DataSourceTypes.Enum.file
                        ? await getSignedFileUrl(isPublicProject
                            ? {
                                projectId,
                                fname: fileName,
                                upload: false,
                    ...
                    updateTotalFileDownloadProgress();
                    const { tableName, rowCount } = await createViewFromFile(fileName, 'main', ds.tableName, downloadedFile);
                ...
                try {
                    ...
                    set((state) => produce(state, (draft) => {
                        draft.dataSourceStates[ds.tableName] = {
                            ...
                            message: err instanceof DuckQueryError
