...
>() {
  return createProjectSlice<PC, SqlEditorState>((set, get) => ({
...
export function useSqlEditorStore<
  PC extends BaseProjectConfig & SqlEditorSliceConfig,
  T,
>(selector: (state: SqlEditorState) => T): T {
  return useBaseProjectStore(
    selector as unknown as (state: ProjectState<PC>) => T,
  );
}
