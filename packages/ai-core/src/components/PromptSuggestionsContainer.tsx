...
type PromptSuggestionsContainerProps = {
  promptSuggestions?: string[];
  isLoading?: boolean;
  className?: string;
};

export const PromptSuggestionsContainer: React.FC<
  PromptSuggestionsContainerProps
> = ({promptSuggestions = [], isLoading = false, className}) => {
  ...
  const handlePromptSuggestionClick = useCallback(
    (promptSuggestion: string) => {
      setAnalysisPrompt(promptSuggestion);
    },
    [setAnalysisPrompt],
  );
  ...
  if ((!promptSuggestions || promptSuggestions.length === 0) && !isLoading) {
    return null;
  }
  ...
  return (
    <TooltipProvider>
      <div
        ...
        className={cn(
          'relative mt-2 flex w-full gap-2 overflow-x-auto overflow-y-hidden p-1',
          isVisible ? 'h-20' : 'h-8',
      ...
      >
        {(isHovered || !isVisible) && (
          <Button
            ...
            size="icon"
            className={cn(
              'text-muted-foreground hover:text-foreground absolute left-1 z-10 h-6 w-6 shrink-0',
        ...
        {isVisible && (
          <>
            {isLoading
              ...
              : promptSuggestions.map((promptSuggestion, index) => (
                  <Tooltip key={index}>
                    <TooltipTrigger asChild>
                      <Button
                        onClick={() =>
                          handlePromptSuggestionClick(promptSuggestion)
                        }
                        className={cn(
                      ...
                      >
                        ...
                        <span className="line-clamp-2 text-wrap break-words">
                          {truncateText(promptSuggestion)}
                        </span>
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>
                      <p className="max-w-xs">{promptSuggestion}</p>
                    </TooltipContent>
                  </Tooltip>
                ))}
          </>
