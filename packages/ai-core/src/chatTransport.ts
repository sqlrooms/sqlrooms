 ...
 */
export function completeIncompleteToolCalls(messages: UIMessage[]): UIMessage[] {
  return messages.map((message) => {
    ...
    const isToolPart = (part: unknown): part is ToolPart => {
      ...
      const typeVal = typeof p.type === 'string' ? (p.type as string) : undefined;
      return !!typeVal && 'toolCallId' in p && (typeVal === 'dynamic-tool' || typeVal.startsWith('tool-'));
    };
    ...
    for (let i = updatedParts.length - 1; i >= 0; i--) {
      ...
      updatedParts[i] = syntheticPart as unknown as (typeof message.parts)[number];
    }
...
export function createLocalChatTransportFactory({
  ...
  return () => {
    const fetchImpl = async (_input: RequestInfo | URL, init?: RequestInit) => {
      ...
      const messagesCopy = Array.isArray(parsedObj.messages) ? (parsedObj.messages as UIMessage[]) : [];
      ...
      try {
        ...
        if (tool && state.ai.tools[toolName]?.execute && tool.execute) {
          ...
          const sessionMessages = (
            state.ai.getCurrentSession()?.uiMessages ?? []
          ) as UIMessage[];
    ...
    onChatFinish: ({messages}: {messages: UIMessage[]}) => {
      try {
        ...
        if (aborted) {
          ...
          const sessionMessages =
            (store.getState().ai.getCurrentSession()?.uiMessages as UIMessage[]) || [];
        ...
        store.setState((state: AiSliceState) =>
          produce(state, (draft: AiSliceState) => {
            ...
            if (targetSession) {
              ...
              const existingMessages = (targetSession.uiMessages || []) as UIMessage[];
