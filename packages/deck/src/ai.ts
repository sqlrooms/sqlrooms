...
- LAYER SELECTION: Choose the layer type based on the geometry type in the data.
  ...
  - Point data (lon/lat coordinates, point geometry): GeoArrowScatterplotLayer (Point layer), GeoArrowHeatmapLayer, GeoArrowColumnLayer. Requires point positions — lon/lat columns (via ST_Point transformSql) or a Point geometry column. Do NOT bind these layers to Polygon/MultiPolygon geometry (typical GeoJSON building footprints, parcels, boundaries). The runtime will not invent centroids. If the user asks for points/heatmap/columns from polygon footprints, you MUST either (a) use GeoArrowPolygonLayer / GeoArrowSolidPolygonLayer instead, or (b) add transformSql that produces points explicitly, e.g. "SELECT * EXCLUDE (geom), ST_AsWKB(ST_Centroid(geom)) AS geom FROM ${DECK_TABLE_DATASET_SOURCE_RELATION}" (or ST_PointOnSurface). Never author a Scatterplot/Heatmap/Column map on raw polygon geom without that SQL.
  - Polygon data (building footprints, boundaries, areas, parcels, zones, GeoJSON polygons): GeoArrowPolygonLayer or GeoArrowSolidPolygonLayer. Requires a geometry column containing ONLY polygon or multipolygon WKB/GeoArrow data (uniform type). Typically loaded from GeoJSON/Shapefile/GeoParquet or produced by spatial queries. Prefer these layers for GeoJSON polygon tables. Do NOT use for point data or mixed geometry columns.
  - MIXED geometry columns (Point + LineString + Polygon in the same geom column — e.g. sample_mixed_geometries, or a "kind"/type column distinguishing feature classes): use GeoJsonLayer with _sqlroomsBinding — NOT GeoArrowPolygonLayer, GeoArrowPathLayer, or GeoArrowScatterplotLayer. Typed GeoArrow* layers require a uniform geometry type and will fail on mixed WKB. If the user wants only one class from a mixed table, filter in transformSql (e.g. WHERE ST_GeometryType(geom) IN ('POLYGON','MULTIPOLYGON')) then use the matching typed layer.
  - Line data (roads, routes, paths, rivers): GeoArrowPathLayer. CRITICAL: GeoArrowPathLayer requires LineString or MultiLineString geometry, NOT individual point rows. If the table has one row per waypoint (indicated by columns like path_id/route_id + order/sequence + lat/lon), you MUST aggregate them with transformSql: "SELECT path_id, label, ST_AsWKB(ST_MakeLine(LIST(ST_Point(lon, lat) ORDER BY waypoint_order))) AS geom FROM ${DECK_TABLE_DATASET_SOURCE_RELATION} GROUP BY path_id, label". Set geometryColumn to "geom" and geometryEncodingHint to "wkb". If the table already has a geometry/geom column with linestring data, use it directly with tableName (or "SELECT * EXCLUDE (geom), ST_AsWKB(geom) AS geom FROM ${DECK_TABLE_DATASET_SOURCE_RELATION} WHERE ST_GeometryType(geom) IN ('LINESTRING','MULTILINESTRING')"). NEVER pass raw waypoint rows to GeoArrowPathLayer — it will fail.
  ...
  - H3 hexagon data (h3 index column): GeoArrowH3HexagonLayer. Bind dataset; set _sqlroomsBinding.hexagonColumn and getHexagon to a string accessor like "@@=hex_id" (not an object/columnAccessor). Include fitToData {"dataset":"datasetId"}. Valid H3 helpers: h3_cell_to_lat/lng/latlng — not h3_latlng/h3_to_lat. When aggregating lon/lat into H3, prefer a hex-string column: "SELECT h3_h3_to_string(h3_latlng_to_cell(lat, lon, 8)) AS h3_cell, COUNT(*) AS trip_count FROM ${DECK_TABLE_DATASET_SOURCE_RELATION} GROUP BY h3_cell" (argument order is lat, lon). BIGINT cell ids also work at runtime, but hex strings match sample H3 datasets and avoid decimal-string confusion.
...
- CRITICAL: The transformSql and sqlQuery fields must contain ONLY a single SELECT statement. NEVER put INSTALL, LOAD, CREATE, or other DDL/meta-commands in dataset SQL — they will fail because dataset SQL is wrapped in a subquery at runtime. Extensions like h3 and spatial are pre-loaded at startup.
  - GeoJSON files typically contain polygon or multipolygon features (boundaries, buildings, parcels); use GeoArrowPolygonLayer / GeoArrowSolidPolygonLayer for these. If the user asked for "points" but the table is polygon footprints, either keep a polygon layer or add ST_Centroid / ST_PointOnSurface in transformSql before using Scatterplot/Heatmap/Column — do not bind point layers to raw polygon geom. If a GeoJSON file contains point features, use GeoArrowScatterplotLayer (Point layer) instead.
...
- HEATMAP: For GeoArrowHeatmapLayer, omit colorRange (UI scheme selector owns it). Omit getWeight — use default density (each point contributes equally). Do NOT bind getWeight to a column; the settings panel has no weight-column control, and column weights are not supported in basic mode.
...
- ELEVATION: For extruded layers set "extruded": true. Prefer getElevation {"@@function":"scale","field":"...","type":"linear","domain":"auto","range":[0,200]} (basic-mode friendly) or "@@=columnName". elevationScale multiplies raw field meters — keep max visual height moderate (~hundreds of meters). Do NOT use negative elevation. For polygon building footprints with GeoArrowColumnLayer, transformSql should produce points: "SELECT * EXCLUDE (geom), ST_AsWKB(ST_Centroid(geom)) AS geom FROM ${DECK_TABLE_DATASET_SOURCE_RELATION}" (or ST_PointOnSurface). CRITICAL: Extruded ColumnLayer / polygon maps need a pitched camera — set spec.initialViewState with pitch around 45–60 (and optional bearing). Top-down pitch 0 makes columns look like flat disks and appear "missing".
...
- CRITICAL: Prefer transformSql that emits WKB with ST_AsWKB(...). Bare ST_Point(...) AS col yields DuckDB GEOMETRY; the dataset pipeline projects GEOMETRY columns to WKB automatically, but explicit ST_AsWKB remains clearer. Never write SELECT *, ST_AsWKB(geom) AS geom — DuckDB keeps the original geom and renames the WKB (geom_1). Prefer no transformSql when geom already exists, or SELECT * EXCLUDE (geom), ST_AsWKB(...) AS geom FROM ${DECK_TABLE_DATASET_SOURCE_RELATION}.
...
- COLOR SCALE PREFERENCE: Prefer colorScale over flat fill when a useful column exists — numeric → sequential/quantile (quantile if skewed); categorical/string → categorical. Flat fill only if no useful column or the user asks for one color. "field" must be the exact schema column name (case-sensitive; "Magnitude" not "mag"). IMPORTANT: Viridis/Plasma/Inferno/Turbo/Cividis/Magma require type "sequential" (not quantile/quantize). Quantile/quantize schemes must be ColorBrewer ramps such as YlOrRd, Blues, Greens, RdYlBu.
...
- BASEMAPS: Omit mapStyle unless the user explicitly requests a custom basemap. Maps inherit the host application's theme-aware basemap when available, with a token-free fallback. PROHIBITED: never set mapStyle to a mapbox:// URL (e.g. mapbox://styles/mapbox/dark-v11) — MapLibre cannot fetch that scheme and the map fails to load. For an explicitly requested custom basemap, use only a token-free MapLibre-compatible https:// style URL.
`;
