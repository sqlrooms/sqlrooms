...
- LAYER SELECTION: Choose the layer type based on the geometry type in the data.
  ...
  - Point data (lon/lat coordinates, point geometry): GeoArrowScatterplotLayer (Point layer), GeoArrowHeatmapLayer, GeoArrowColumnLayer. Requires rows with individual point positions — either separate longitude/latitude numeric columns, or a point geometry column. Each row represents one point on the map. For polygon footprints (buildings, parcels) that should render as extruded columns, either use GeoArrowPolygonLayer/GeoArrowSolidPolygonLayer, or transformSql with ST_AsWKB(ST_Centroid(geom)) / ST_PointOnSurface(geom) and bind that point column to GeoArrowColumnLayer.
