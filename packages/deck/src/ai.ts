...
- CRITICAL: Never write transformSql as "SELECT *, ST_AsWKB(geom) AS geom FROM ..." — DuckDB keeps the original geom and renames the WKB (geom_1), so polygons/points render empty. Prefer no transformSql when geom already exists. When ST_AsWKB (or ST_Centroid) is required, use "SELECT * EXCLUDE (geom), ST_AsWKB(...) AS geom FROM ${DECK_TABLE_DATASET_SOURCE_RELATION}".
