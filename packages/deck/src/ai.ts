...
- For data-driven color, use native Deck JSON accessors with {"@@function":"colorScale", "field":"...", "type":"sequential"|"diverging"|"quantize"|"quantile"|"categorical", "scheme":"...", "domain":"auto"} on color properties such as getFillColor, getLineColor, getColor, getSourceColor, or getTargetColor. CRITICAL: The key is "@@function" (not "@@type") and the column name goes in "field" (not "column"). Using "@@type":"ColorScale" or "column" instead of "field" is wrong syntax that will fail. Valid schemes (use EXACT names, they are case-sensitive): ${COLOR_SCHEME_PROMPT_LISTS} ONLY use names from these exact lists — do not invent names like "bluepurple" or "blue-purple". IMPORTANT: The colorScale "field" must reference a column that exists in the FINAL query output (after any GROUP BY aggregation). Do not reference columns that are lost during aggregation.
