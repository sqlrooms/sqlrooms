...
- IMPORTANT: Dashboard map tools replace the panel's full config. When updating an existing dashboard map, always send a complete config including datasets and layers — do NOT omit config.datasets. Omitting datasets is only valid for worksheet/resource sparse patches that merge with durable state (see the direct map resource instructions). Sending a dashboard update without datasets leaves layers unbound and breaks the map.
- SWITCHING LAYER TYPE: When the user asks to "switch", "change", or "replace" a layer type (e.g. point → heatmap, scatterplot → column), send only the new layer and set replaceLayers: true when using a merge-capable resource patch. For dashboard full-config updates, send the complete desired layers list (without keeping the old layer hidden via visible: false).
