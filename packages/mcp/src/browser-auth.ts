/** Explicit, tab-local authorization for the configured local application instance. */
...
/** Create isolated tab-local browser authorization for a local application. */
export function createBrowserAuthorization({
  ...
  function authorizationHeaders(url: string | URL): Record<string, string> {
    ...
    if (
      ...
      throw new Error('Refusing to send page credentials to another origin.');
    }
  ...
  async function bootstrapAuthorization(): Promise<void> {
    authorization = undefined;
    clearTimeout(renewalTimer);
    const params = new URLSearchParams(location.hash.slice(1));
    const ticket = params.get(`${product}-ticket`);
    if (ticket)
    ...
    storageKey = `${product}:authorization:${location.origin}:${binding}:page`;
    try {
      const prefix = `${product}:authorization:${location.origin}:`;
  ...
  function pageCredential(): string {
