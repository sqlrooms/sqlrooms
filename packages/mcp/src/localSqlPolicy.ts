...
export async function inspectLocalSelect(
...
export function assertLocalDestination(
  db: DuckDbSliceState['db'],
  tableName: string,
  namespace: string,
): QualifiedTableName {
  ...
  if (!parsed?.table)
    throw new LocalSqlError(
      'invalid_table',
      'Use a valid SQL table identifier.',
    );
...
export async function needsLocalReadApproval(
  db: DuckDbSliceState['db'],
  statements: unknown[],
): Promise<boolean> {
