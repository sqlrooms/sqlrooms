import {
  ...
  DEFAULT_QUERY_ROWS,
  MAX_QUERY_ROWS,
} from './localMcpToolContract';
import type {RoomCapability, RoomCapabilityContext} from './index';
import {
  arrowTableToJson,
  type DataTable,
  getTableDisplayName,
...
type CreateLocalRoomCapabilitiesOptions = {
  approveOperation?: (
    operation: LocalOperationApproval,
    context: RoomCapabilityContext,
  ) => Promise<'allow' | 'deny' | 'cancelled' | 'expired'>;
  store: StoreApi<RoomShellSliceState>;
  metaNamespace?: string;
  /** Restricts discovery and execution to the application's command surface. */
  commandFilter?: (command: RoomCommandDescriptor) => boolean;
...
export function createLocalRoomCapabilities({
  store: roomStore,
  ...
  function createQueryCapability(metaNamespace: string): RoomCapability {
    return {
      ...
      execute: async (rawInput, context) => {
        ...
        try {
          const parsed = await inspectLocalSelect(state.db, sql, metaNamespace);
          if (await needsLocalReadApproval(state.db, parsed)) {
          ...
          return {
            ok: false,
            code:
              error instanceof LocalSqlError
                ? error.code
  ...
  function createReadTableSchemaCapability(
    ...
    return {
      ...LOCAL_MCP_TOOLS.read_table_schema,
      execute: async (rawInput) => {
  ...
  function createSearchCommandsCapability(): RoomCapability {
    return {
      ...LOCAL_MCP_TOOLS.search_commands,
  ...
  function createExecuteCommandCapability(): RoomCapability {
    return {
      ...LOCAL_MCP_TOOLS.execute_command,
      execute: async (rawInput, context) => {
  ...
  function listMcpCommands(
    ...
    return roomStore
      ...
      .filter(
        (command) =>
          !MCP_EXCLUDED_COMMAND_IDS.has(command.id) &&
          (commandFilter?.(command) ?? true),
      )
