...
import {z} from 'zod';
import {assertLocalDestination, inspectLocalSelect} from './localSqlPolicy';
import type {RoomShellSliceState as RoomState} from '@sqlrooms/room-shell';

/** Host adapter resolves a local file on the machine running DuckDB. */
export type LocalFileResolver = (
...
const importInput = z
  .object({
    ...
    path: z
      ...
      .min(1)
      .describe(
        'Local file path on the workspace server; ~ is expanded there. Do not use file://.',
      ),
...
export function createLocalDataCommands(
  options: {
  ...
  return [
    {
      ...
      validateInput: async (raw, {getState}) => {
        ...
        assertLocalDestination(getState().db, input.tableName, namespace);
        await inspectLocalSelect(getState().db, input.query, namespace);
      },
      execute: async ({getState, signal}, raw) => {
        const input = createInput.parse(raw);
        // Validate again at the execution boundary, including direct invocation.
        const target = assertLocalDestination(
          getState().db,
          input.tableName,
          namespace,
        );
