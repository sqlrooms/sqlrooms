 ...
 */
export function createRoomStore<PC, AppState extends RoomState<PC>>(
  stateCreator: StateCreator<AppState>,
) {
  const roomStore = createStore<AppState>((set, get, store) => ({
  ...
  function useRoomStore<T>(selector: (state: AppState) => T): T {
    // @ts-ignore TODO fix typing
    return useBaseRoomStore(selector as (state: AppState) => T);
  }
