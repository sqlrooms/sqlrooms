...
async def test_ws_subscribe_notify(server_proc):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.ws_connect(f'ws://localhost:{port}') as ws:
            await ws.send_str(json.dumps({'type': 'subscribe', 'channel': 'table:orders'}))
            # ack
            msg = await ws.receive()
            ...
            # publish
            await ws.send_str(json.dumps({'type': 'notify', 'channel': 'table:orders', 'payload': {'op': 'update'}}))
            ...
            for _ in range(20):
                ...
                if msg.type == aiohttp.WSMsgType.TEXT:
                    try:
                        ...
                        continue
                    if payload.get('type') == 'notify' and payload.get('channel') == 'table:orders':
