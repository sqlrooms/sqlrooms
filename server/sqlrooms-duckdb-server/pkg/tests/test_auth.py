...
async def test_ws_auth_flow(server_proc_auth):
            ...
            for _ in range(10):
                ...
                if msg.type == aiohttp.WSMsgType.TEXT:
                    ...
                    try:
                        ...
                        continue
                    if payload.get('type') == 'error' and 'unauthorized' in str(payload.get('error', '')).lower():
                        got_unauth = True
                        break
                elif msg.type in (aiohttp.WSMsgType.CLOSE, aiohttp.WSMsgType.CLOSED, aiohttp.WSMsgType.CLOSING, aiohttp.WSMsgType.ERROR):
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        async with session.ws_connect(f'ws://localhost:{port}') as ws:
            ...
            qid = 'auth_ok_1'
            await ws.send_str(json.dumps({'type': 'json', 'sql': 'select 1 as x', 'queryId': qid}))
            got_result = False
            query_debug = []
            for _ in range(200):
                msg = await ws.receive()
                    ...
                    if len(buf) >= 4:
                        ...
                        if 4 + hlen <= len(buf):
                            try:
                                ...
                                query_debug.append(f"BINARY HEADER: {json.dumps(header)}")
